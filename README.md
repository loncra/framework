...
| **持久层** | [`spring-boot-starter-mybatis`](starter/spring-boot-starter-mybatis/README.md)、[`spring-boot-starter-mybatis-plus`](starter/spring-boot-starter-mybatis-plus/README.md)；含写操作留痕与可选 **`OperationDataTraceRecordHook`**（构建阶段 `preCreate`/`postCreate`、保存前 `preSave`，规则见 mybatis README） |
...
| **spring-boot-starter-mybatis** | MyBatis TypeHandler、JSON 集合拦截、写操作留痕与 **`OperationDataTraceRecordHook`**（表级钩子） | [starter/…/README.md](starter/spring-boot-starter-mybatis/README.md) |
| **spring-boot-starter-mybatis-plus** | MP 插件链、加密、多租户、默认留痕解析器（聚合 `OperationDataTraceRecordHook`）等 | [starter/…/README.md](starter/spring-boot-starter-mybatis-plus/README.md) |
